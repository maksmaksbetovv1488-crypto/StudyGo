...
def init_db():
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            # ─── Channel subscription checks (optional cache) ────────────────
            cur.execute("""
...
def _calc_level(score: int) -> int:
    ...
    for lvl, threshold in LEVEL_THRESHOLDS:
        if score >= threshold:
            ...
            break
    # Fill intermediate levels linearly for nicer progression
    # Simple approach: find highest defined and interpolate
...
def confirm_referral(referred_id: int) -> Optional[int]:
    ...
    # Award outside of the previous transaction lock for clarity
...
def close_daily_rank_and_reward():
    ...
    # Also save places beyond top-3 for history (optional, top-30)
