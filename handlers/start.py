...
from database import (
    ensure_user, is_blocked, create_referral, confirm_referral, get_pending_referral,
)
...
async def cmd_start(message: Message, command: CommandObject):
    ...
    if pending:
        await message.answer(
            ...
            "пригласившему начислятся бонусы.",
    ...
    await message.answer(
        "📚 <b>STUDYGO</b>\n\n"
        "Школьный AI-помощник.\n"
        "Тебе уже выдан <b>Pro на 3 дня</b> 🎁\n"
        "Решай задачи, бери бонусы, оформи подписку!",
...
async def cb_check_sub(callback: CallbackQuery, bot: Bot):
    ...
    if await check_channel_subscription(bot, user_id):
        ...
        if referrer:
            text += "Реферал засчитан — пригласивший получил +3 дня Free+ 🎉\n"
        text += "\nДобро пожаловать! У тебя Pro на 3 дня."
